...
LIBRESPOT_NAME = os.environ.get("LIBRESPOT_NAME") or os.environ.get(
    "LIBRESPOT_DEVICE_NAME",
)
if not LIBRESPOT_NAME:
    LIBRESPOT_NAME = "CycleMusicLibrespot"
...
def capture_playlist_tracks(playlist_id_or_url: str, captures_dir: str | os.PathLike) -> None:
    """
    Entrypoint expected by the outer scheduler.

    This will block until all tracks present in the given playlist have
    been captured into `captures_dir` (or until interrupted).
    """
    global OUT_DIR
    ...
    if not (client_id and client_secret and refresh_token):
        raise KeyError(
            "SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET and SPOTIFY_REFRESH_TOKEN must be set in the environment"
        )

    playlist_id = extract_playlist_id(playlist_id_or_url)
    print(f"Capturing playlist: {playlist_id} → {OUT_DIR}")

    token = refresh_access_token(client_id, client_secret, refresh_token)
    target_ids = fetch_playlist_track_ids(token, playlist_id)
    remaining = set(target_ids)
    print(f"Found {len(remaining)} tracks to capture from playlist")

    # Start librespot keepalive so the device advertises over Zeroconf
    librespot_proc = start_librespot()

    # If some tracks already exist in captures_dir, skip them
    for f in OUT_DIR.glob("*.wav"):
        sid = extract_spotify_id(f.name)
        if sid and sid in remaining:
            remaining.discard(sid)

    print(f"{len(remaining)} tracks remain to capture")

    # Main capture loop: similar logic to main(), but stops when remaining is empty
    spotify_config = {
        "client_id": client_id,
    ...
    try:
        while remaining:
            ...
            track_id = item.get("id")
            if not track_id or track_id not in remaining:
                time.sleep(POLL_SECONDS)
                continue
            ...
            # Verify file and move
            if tmp_wav.exists() and tmp_wav.stat().st_size > 100_000:
                final_wav = OUT_DIR / tmp_wav.name
                try:
                    import shutil

                    shutil.move(str(tmp_wav), str(final_wav))
                    print(f"Saved capture: {final_wav.name}")
                    # Run analysis synchronously to be deterministic for the scheduler
                    run_analyzer(final_wav)
                except Exception as e:
                    print(f"Failed moving/analyzing {tmp_wav}: {e}")
                remaining.discard(track_id)
            else:
                print(f"Captured file for {track_id} too small; skipping")
